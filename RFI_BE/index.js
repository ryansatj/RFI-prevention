        ...
        <html>
            ...
            <body>
                ...
                <div class="button-container">
                    <a href="/template/login.html" class="nav-button">Login</a>
                    <a href="/template/register.html" class="nav-button">Register</a>
                    <a href="/template/blank.html" class="nav-button">Blank Page</a>
                </div>
            </body>
        </html>
    `);
...
app.get('/execute/:filename', (req, res) => {
    ...
    if (fs.existsSync(filePath)) {
        ...
        if (fileExtension !== '.html') {
            return res.status(400).send('Invalid file type. Only .html files can be served.');
        }
        ...
        try {
            const compiledHtmlContent = compileHtmlContent(fileContent);
            
            const scriptFunctionPath = filePath.replace('.html', '.js'); 
            if (fs.existsSync(scriptFunctionPath)) {
                const scriptFunction = require(scriptFunctionPath);  
                scriptFunction(req, res, compiledHtmlContent);  
            } else {
                res.send(compiledHtmlContent);
            }
...
function compileHtmlContent(parameter) {
    const htmlContent = `${parameter}`; // Template literal to inject the parameter
    return htmlContent;
}
